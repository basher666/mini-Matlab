...
primary_expression	
	: IDENTIFIER {	/*
					...
					*/
					...
					$$->symp = $1;
					$$->isbool = false;
					if(table->search($1->name))
					{
						//cout<<"IDFOUND"<<endl;
						$$->dims=table->lookup($1->name)->dims;
					}
					if(gDebug)
					{
...
constant 		//creates a new temporary for each constant and assigns it to lhs non-terminal for further propagation
	...
	if(gDebug)
		...
		emit(EQUALCHAR, $$->name, char_quote(yytext[1]));
	}
	;
...
postfix_expression
	...
	*/
		...
		$$ = new unary_expr ();
		$$->t_list=$1->truelist; //change-log
		$$->f_list=$1->falselist; //change-log
		$$->n_list=$1->nextlist;  //change-log
		$$->bool_is=$1->isbool;  //change-log
		...
		if($1->symp!=NULL)   //change-log
		{
			...
			{
				//cout<<"4"<<endl;
				$$->symp->size=$1->symp->size;
				$$->cat=$1->symp->type->cat;

				if($$->dims.size()==0 && $1->dims.size()>0)
				{
					$$->dims=$1->dims;
					//cout<<$$->dims[0]<<endl;
					//cout<<"hoolabula"<<endl;
					//cout<<$$->dims[1]<<"hhh"<<endl;
				}
			}
		}
		//cout<<"5"<<endl;
...
multiplicative_expression
	: cast_expression 		
	{
		...
		{	//Matrix handlings
			...
			$$->symp = gentemp("",$1->type);
			if($$->symp->dims.size()==0 && $1->symp->dims.size()>0)
			{
				//cout<<"casst"<<endl;
				//cout<<$1->symp->name<<endl;
				//cout<<"castafaf"<<endl;
				$$->symp->dims=$1->symp->dims;
				//cout<<$$->symp->dims[0]<<","<<$$->symp->dims[1]<<endl;
			}
			if($1->start_trans)
			{
				...
				ST_entry* t_util = gentemp("",_DOUBLE);

				for(int ii=0;ii<dim1;ii++)
				{
					for(int jj=0;jj<dim2;jj++)
					{
						emit(ARRR_I , t_util->name, $1->symp->name, num_to_string((ii*dim2+jj)*8));
						emit(ARRL_I , $$->symp->name , num_to_string((jj*dim1+ii)*8), t_util->name);
					}
				}
		...
		else             			//change-log can improve this portion
		{ 
			//default
			$$->symp = $1->symp;
			$$->truelist=$1->t_list;
	...
	| multiplicative_expression '*' cast_expression 	
	{
		...
		if (typecheck ($1->symp, $3->symp) )
		{
			if($1->symp->type->cat==MAT)
			{
				...
				if($1->symp==NULL)
					//cout<<"nmmmw"<<endl;
				//cout<<"maybeexception"<<$1->symp->dims[0]<<endl;

				$$->symp->type->width=$1->symp->dims[0];
				$$->symp->type->ptr->width=$3->symp->dims[1];
				

				//cout << $$->symp->type->width << "," << $$->symp->type->ptr->width <<endl;
				$$->symp->dims.push_back($1->symp->dims[0]);
				$$->symp->dims.push_back($3->symp->dims[1]);
				
				if($1->symp->dims[1]!=$3->symp->dims[0])
				{
					cout<<"size mismatch while multiplying"<<endl;    //never gonna happen
					exit(1);
				}
				//cout << next_instruction() <<endl;
				//cout <<"$1 - > "<< $1->symp->dims[0]<<$1->symp->dims[1]<<endl;
				//cout <<"$$ - > "<< $$->symp->dims[0]<<$$->symp->dims[1]<<endl;

				int p = $1->symp->type->width;
	     	  	int q = $3->symp->type->width;
				int r = $3->symp->type->ptr->width;

				//cout <<"p=" << p << "q=" << q <<"r=" <<r<<endl;

				ST_entry * t_1 = gentemp("", _DOUBLE);
				ST_entry * t_2 = gentemp("", _DOUBLE);
				ST_entry * t_3 = gentemp("" , _DOUBLE);
				ST_entry * tmp_sum = gentemp("",_DOUBLE);
				int tmp_ind=0;
				for(int ii=0;ii<p;ii++)
				{
					for(int jj=0;jj<r;jj++)
					{
						emit(ZERO_DOUBLE,tmp_sum->name,num_to_string(0));
						for(int kk=0;kk<q;kk++)
						{
							//cout << "now"<<endl;
							//cout << "(ii*q+kk)*8->" << (ii*q+kk)*8 << endl;
							//cout << "(kk*r+jj)*8->" << (kk*r+jj)*8 << endl;
							emit(ARRR_I,t_1->name,$1->symp->name,num_to_string((ii*q+kk)*8));
							emit(ARRR_I,t_2->name, $3->symp->name, num_to_string((kk*r+jj)*8));
							emit(MULT,t_3->name,t_1->name,t_2->name);
							emit(ADD,tmp_sum->name,tmp_sum->name,t_3->name);
						}
						//cout << "ii= "<<ii <<",jj= "<<jj<<endl;
						//cout << "(ii*q+jj)*8->" << (ii*q+jj)*8 <<endl;
						emit(ARRL_I,$$->symp->name,num_to_string(tmp_ind),tmp_sum->name);
						tmp_ind+=8;
					}
				}
	...
	| additive_expression '+' multiplicative_expression 		//handles addition of primitive and matrix data type , throws type mismatch error in case of mismatch
	{
		...
		if (typecheck($1->symp, $3->symp))
		{
			if($1->symp->type->cat==MAT)
			{
				...
				//emit(MAT_ADD, $$->symp->name , $1->symp->name , $3->symp->name);
				ST_entry * add_tmp=gentemp("",_DOUBLE);
				ST_entry * add_tmp2=gentemp("",_DOUBLE);
				ST_entry * add_tmp3=gentemp("",_DOUBLE);

				for(int ii=0;ii < max(table->lookup($1->symp->name)->size,table->lookup($$->symp->name)->size);ii+=8)
				{
					//cout<<"adddmat"<<endl;
					//cout <<$1->symp->name<<","<<$3->symp->name<<","<<$$->symp->name<<endl;
					emit(ARRR_I, add_tmp->name, $1->symp->name, num_to_string(ii));
					emit(ARRR_I, add_tmp2->name, $3->symp->name, num_to_string(ii));
					emit(ADD, add_tmp3->name, add_tmp->name,add_tmp2->name);
					emit(ARRL_I, $$->symp->name, num_to_string(ii) , add_tmp3->name);
				}
			}
			else
			{
	...
	| additive_expression '-' multiplicative_expression 		// subtraction rule for additive expression ,creates a temporary and emits the necessary quads
	{
		...
		if (typecheck($1->symp, $3->symp))
		{
			if($1->symp->type->cat==MAT)
			{
				$$=new expr_struct();
				$$->symp=gentemp("",$1->symp->type);
				//emit(MAT_SUB, $$->symp->name , $1->symp->name , $3->symp->name);
				ST_entry * sub_tmp=gentemp("",_DOUBLE);
				ST_entry * sub_tmp2=gentemp("",_DOUBLE);
				ST_entry * sub_tmp3=gentemp("",_DOUBLE);
				for(int ii=0;ii < max(table->lookup($1->symp->name)->size,table->lookup($$->symp->name)->size);ii+=8)
				{
					//cout<<"subbmat"<<endl;
					//cout <<$1->symp->name<<","<<$3->symp->name<<","<<$$->symp->name<<endl;
					emit(ARRR_I, sub_tmp->name, $1->symp->name, num_to_string(ii));
					emit(ARRR_I, sub_tmp2->name, $3->symp->name, num_to_string(ii));
					emit(SUB, sub_tmp3->name, sub_tmp->name,sub_tmp2->name);
					emit(ARRL_I, $$->symp->name, num_to_string(ii) , sub_tmp3->name);
				}
			}
			else
			{
...
logical_AND_expression 		// required for logical AND expressions
	...
	| logical_AND_expression LOGIC_AND M inclusive_OR_expression 
	{
		convert_to_bool($4);
...
logical_OR_expression
	...
	| logical_OR_expression LOGIC_OR M logical_AND_expression 
	{
		//cout<<"hh"<<endl;
		convert_to_bool($4);
...
conditional_expression
	...
	| logical_OR_expression N '?' M expression N ':' M conditional_expression 
	{
		...
		//cout<<"M4="<<$4<<endl;
		//cout<<"M8"<<$8<<endl;
		//cout<<"truelist size="<<$1->truelist.size()<<endl;
		$$->symp = gentemp("",_INT); 
	...
	| unary_expression assignment_operator assignment_expression
	{
		...
		switch ($1->cat)
		{
			...
			case MAT:
				...
				if($1->symp->size == 4 && $3->symp->size!=4)
				{
					$1->symp->size = $3->symp->size; //maybe
				}
				if($1->symp->size != 4 && $3->symp->size==4)
				{
					$3->symp->size = $1->symp->size; //maybe
				}
				break;
			default:
...
direct_declarator
	...
	| direct_declarator '[' assignment_expression ']' 
	{
		...
		if(table->search($1->name))
		{
			table->lookup($1->name)->dims.push_back(atoi($3->symp->init.c_str())) ;
...
initializer_row_list  // this handles the matrix initializations 
	: initializer_row 				
		{
			$$->mat_init_col_list.push_back($1->mat_init_row_list);
