...
bool symbol_table::search (string name)
{
	ST_entry* s=NULL;
	list <ST_entry>::iterator it=table.begin();
	for (; it!=table.end(); it++)
...
void AR(symbol_table* stmp , int param  , int local )
{
	for (list <ST_entry>::iterator it = stmp->table.begin(); it!=stmp->table.end(); it++) 
	{
		if (it->category =="param") 
		{
			stmp->ar [it->name] = param;
			param +=it->size;			
		}
		...
		else 
		{
			if(it->type->cat==MAT)
			{
				stmp->ar[it->name]=local-it->size+8;
				local -= it->size;
			}
			else
			{
				stmp->ar [it->name] = local;
				local -=it->size;
			}
		}
	}
...
void quad::print () 
{
	switch(op)
	{
		...
		case INIT_MAT:		cout << result << "[" << arg1 << "]" << "=" << arg2; 			break;
		case MAT_ADD:		cout << result << " = " << arg1 << "(+)" << arg2; 				break;
		case MAT_SUB:		cout << result << " = " << arg1 << "(-)" << arg2; 				break;
		case MAT_MUL:		cout << result << " = " << arg1 << "(*)" << arg2; 				break;
		case ARRR:	 		cout << result << " = " << arg1 << "[" << arg2 << "]";			break;
		case ARRL:	 		cout << result << "[" << arg1 << "]" <<" = " <<  arg2;			break;
		case ARRL_I:		cout << result << "[" << arg1 << "]" <<" = " << arg2;			break;
		case ARRR_I:	 		cout << result << " = " << arg1 << "[" << arg2 << "]";			break;
		case TRANSPOSE:		cout << result << " = " << arg1 << ".'";	break;
		case _RETURN: 		cout << "ret " << result;				break;
		case PARAM: 		cout << "param " << result;				break;
		case ZERO_DOUBLE:	cout << result << " = 0 " ;		break;
		case CALL: 			cout << result << " = " << "call " << arg1<< ", " << arg2;				break;
		case FUNCEND:		break;
		case EQUALSTR:		cout << result << " = " << arg1; 			break;
		case EQUAL_DOUBLE:	cout << result << " = " << arg1; 			break;
		case FUNC:			cout << result << ": ";					break;
		default:			cout << "op"<<op;							break;
	}
	// switch(op)
	// {
...
void for2_handler(statement* s0 ,int i4,expr_struct* e5, int i6, expr_struct* e8, int i10,statement* s11)
{
	...
	backpatch(e5->falselist, next_instruction()+1);
...
void do_while_handler(statement* s0 ,int i2, statement* s3, int i4,expr_struct* e7)
{
	...
	backpatch(e7->falselist, next_instruction());
}
...
*/
void if_handler(statement* s0, expr_struct* e3, int i5, statement* s6, expr_struct* e7)
{
	backpatch (e7->nextlist, next_instruction());
	convert_to_bool(e3);
	backpatch (e3->truelist, i5);
	backpatch (e3->falselist, e7->nextlist.front());
	list<int> temp = merge (e3->falselist, s6->nextlist);
	s0->nextlist = merge (e7->nextlist, temp);
}
...
output:	required backpatching of codes and merging as required , returns none
*/
void if_else_handler(statement* s0 ,expr_struct* e3, int i5, statement* s6, expr_struct* e7,int i9,statement* s10)
{
	backpatch (e7->nextlist, next_instruction());
	convert_to_bool(e3);
	backpatch (e3->truelist, i5);
	backpatch (e3->falselist, i9);
	list <int> temp = merge (s6->nextlist, e7->nextlist);
	s0->nextlist = merge (temp, s10->nextlist);
...
void while_handler(statement* s0 , int i2 , expr_struct* e4 , int i6 , statement * s7)
{
	...
	backpatch(e4->falselist, next_instruction());
}
/*
